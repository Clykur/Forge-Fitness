...
export function Navbar() {
  ...
  return (
    <header
      className={`fixed top-9 left-0 right-0 z-40 transition-all duration-300 ${isScrolled ? "bg-background/85 backdrop-blur-md border-b border-white/10 py-3" : "bg-transparent py-5"
        }`}
    >
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`text-sm font-medium transition-colors ${isActive ? "text-primary" : "text-white/70 hover:text-white"
                  }`}
              >
                {link.name}
              </Link>
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`text-lg font-medium p-3 rounded-lg transition-colors ${isActive ? "text-primary bg-primary/10" : "text-white/80 hover:bg-white/5"
                  }`}
